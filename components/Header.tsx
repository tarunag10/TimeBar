  ...
  return (
    <header className="sticky top-0 z-50 bg-[var(--bg-primary)]/86 sm:bg-[var(--bg-primary)]/70 backdrop-blur-[20px]" style={{ borderBottom: '1px solid transparent', borderImage: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.1), rgba(255,255,255,0.05), transparent) 1' }}>
      ...
      {mobileMenuOpen && (
        ...
        >
          <nav aria-label="Mobile navigation" className="max-w-6xl mx-auto px-3.5 py-3 space-y-1">
            <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0 * 0.05 }}>
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2.5 text-[14px] text-slate-400 hover:text-slate-100 hover:bg-[var(--surface-hover)] rounded-lg transition-all duration-200 cursor-pointer min-h-[48px]"
              >
                <HelpCircle className="w-4 h-4" />
                Show help
              </button>
            ...
            {navLinks.map((link, index) => (
              <motion.div key={link.href} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: (index + 2) * 0.05 }}>
                <Link
                  href={link.href}
                  className={`block px-3 py-2.5 text-[14px] font-medium rounded-lg transition-all duration-200 min-h-[48px] leading-7 ${
                    pathname === link.href
